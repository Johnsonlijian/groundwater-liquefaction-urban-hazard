...
def find_repo_root() -> Path:
    ...
    for parent in [here.parent, *here.parents]:
        if (parent / "data_derived").exists() and (parent / "figures").exists():
            return parent
    return here.parents[1]


ROOT = find_repo_root()
DATA = ROOT / "data_derived"
OUT = ROOT / "figures" / "visual_upgrade"
SVG_DIR = OUT
PDF_DIR = OUT
PNG_DIR = OUT
PPT_DIR = OUT / "pptx"
...
def export_svg(name: str, parts: list[str]):
    ...
    if cairosvg is not None:
        try:
            ...
            cairosvg.svg2png(bytestring=svg.encode("utf-8"), write_to=str(path_png), output_width=W, output_height=H)
            return path_svg
        except Exception:
            pass
...
def load_summary():
    display = json.loads((DATA / "article_display_reorder_summary_r42.json").read_text(encoding="utf-8"))
    readiness = pd.read_csv(DATA / "article_readiness_dashboard_r41.csv")
    regional = pd.read_csv(DATA / "regional_hierarchical_evidence_model_r41.csv")
    ladder = pd.read_csv(DATA / "four_product_evidence_ladder_r41.csv")
    cards = pd.read_csv(DATA / "evidence_tier_cards_r39.csv")
    phase = pd.read_csv(DATA / "material_unit_aquifer_class_phase_r39.csv")
    eng = pd.read_csv(DATA / "engineering_susceptibility_enrichment_r37.csv")
    events = pd.read_csv(DATA / "event_hindcast_metrics_r40.csv")
    protocol = pd.read_csv(DATA / "review_protocol_box1_r42.csv")
    cities = pd.read_csv(DATA / "city_results_v2.csv")
...
def build_pptx():
    ...
    for idx, (title, subtitle, items) in enumerate(slide_defs):
        ...
        if idx == 0:
            ...
            add_textbox(slide, 0.9, 4.1, 11.4, 1.2, "recharge → shallower WTD → P_liq ↑     |     depletion → deeper WTD → P_liq ↓ but subsidence/water-security audit", 18, NAVY, True, "#EEF5FF", "#B8C8DD")
        elif idx == 1:
            ...
            add_textbox(slide, 1.0, 4.0, 11.4, 1.0, "Global mean ≈ 0; geographic null p = 1.00. The useful result is regional and bidirectional.", 19, NAVY, True, "#EAF5E8", "#B8D4B4")
        elif idx in {2, 3}:
...
def main():
    figs = [
        ...
        fig4_product_ladder(),
        fig5_sy_phase(),
        fig6_engineering_event(),
    ]
    build_pptx()
    build_contact_sheet(figs)
    review = ROOT / "docs" / "visual_upgrade" / "nature_water_visual_upgrade_pack_review.md"
    review.parent.mkdir(parents=True, exist_ok=True)
    review.write_text(
        "# Nature Water Visual Upgrade Pack Review\n\n"
        "Generated SVG/PDF/PNG upgrade candidates for Figures 1-6 plus an editable PPTX deck.\n\n"
        "Scientific boundary: Fig. 1 uses a 3D mechanism schematic to explain the water-table state-variable pathway only; data-bearing claims remain in the derived-table panels. No AI-generated raster is used as evidence.\n\n"
        "Self-review: PASS for editable vector provenance; REVISION likely needed for final journal typography after author micro-adjustment in SVG/PPT.\n\n"
